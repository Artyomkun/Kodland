...
class AIModel:
    ...
    def _load_or_init_model(self):
        ...
        try:
            if os.path.exists(self.model_path):
                ...
                with getattr(h5py, "File")(self.model_path, 'r') as f:
                    f: File
                    if 'model' in f:
                        dataset: Dataset = getattr(f, "__getitem__")('model')
                        if hasattr(dataset, 'shape') and hasattr(dataset, 'dtype'):
                            loaded_void: np.ndarray[Any, np.dtype[np.void]] = getattr(dataset, "__getitem__")(())
                            model_bytes: bytes = loaded_void.tobytes()
                            buffer = io.BytesIO(model_bytes)
                            self.model = getattr(joblib, "load")(buffer)
    ...
    def save_model(self):
        ...
        if self.model:
            try:
                ...
                buffer = io.BytesIO()
                getattr(joblib, "dump")(self.model, buffer)
                model_bytes = buffer.getvalue()
                with getattr(h5py, "File")(self.model_path, 'w') as f:
                    f: File
                    getattr(f, "create_dataset")('model', data=np.void(model_bytes))
    ...
    def process_message(self, user_id: int, text: str) -> dict[str, str]:
